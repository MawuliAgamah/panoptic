...
class CsvParser(DocumentParser):
    """Parser for CSV files. Returns a light textual representation of rows.

    This is primarily to populate raw_content for metadata/preview; graph construction
    from CSV is handled by a dedicated pipeline step.
    """
    ...
    def parse(self, file_path: str) -> str:
        ...
        try:
            ...
            with open(file_path, "r", encoding="utf-8", newline="") as f:
                # Try to sniff dialect to handle different separators
                sample = f.read(2048)
                f.seek(0)
                try:
                ...
                if not rows:
                    return ""
                headers = rows[0]
                for row in rows[1:6]:  # cap preview
                    pairs = [f"{h}={v}" for h, v in zip(headers, row)]
                    lines.append(", ".join(pairs))
            preview = "\n".join(lines)
            logger.debug("Parsed csv file %s: %d preview rows", file_path, len(lines))
            return preview
        except Exception as exc:
            logger.error("Error parsing csv file %s: %s", file_path, exc)
            return ""
